...
const ArticleList = ({ term }) => {
  ...
  const { data: apiData, error } = useSWR(fetchURL, fetcher);
//search data
  ...
  useEffect(() => {
    const articles = data?.articles?.filter((i) =>
      i.title.toLowerCase().includes(term.toLowerCase())
    );
    const articlesCount = articles?.length;
    const sData = { articles, articlesCount };
    setData(term !== "" ? sData : ogData);
  }, [term]);
  //
  if (error) {
    return (
